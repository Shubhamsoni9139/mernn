                ...
                <div className="relative group w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 p-4" key={item.id}>
                  ...
                  <div className="flex flex-col mt-4 space-y-2">
                    <h3 className="text-xs font-bold text-gray-900 sm:text-sm md:text-base truncate">
                      <a onClick={() => (window.location.href = `/productinfo/${item.id}`)} title={item.title}>
                        {item.title}
                    ...
                    <p className="text-xs font-bold text-gray-900 sm:text-sm md:text-base">₹{item.price}+gst</p>
                  </div>
                </div>
              ))}
          </div>
